Write-host "The following script was developed in Powershell 7.4.1. Azure admin user ID usually your email address for username in this script. This script can impact production work loads if the incorrent selections is made"

# Prompt to acknowledge usage with care
$acknowledge = Read-Host -Prompt "This script will make changes to your Azure resources. Do you acknowledge and agree to use it with care? (yes/no)"
if ($acknowledge -ne "yes") {
    Write-Host "Exiting script. Please ensure you use it with care."
    exit
}
